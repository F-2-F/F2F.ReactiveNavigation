...
namespace F2F.ReactiveNavigation.Internal
{
    ...
    {
        private SemaphoreSlim _routerLock = new SemaphoreSlim(1);
        ...
        private async Task RequestNavigateAsyncInternal<TViewModel>(IRegion region, INavigationParameters parameters)
                where TViewModel : ReactiveViewModel
        {
            if (await _routerLock.WaitAsync(LockTimeout))
            {
                if (parameters.Has("RoomId"))
                {
                    var roomId = parameters.Get<Guid>("RoomId");
                    System.Diagnostics.Debug.WriteLine($"ROUTER ACQUIRED LOCK >>>>>>>>>>>>>>>>>>>>>>>>>>>>>  Room id: {roomId}");
                }
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            var roomId = parameters.Get<Guid>("RoomId");
                            System.Diagnostics.Debug.WriteLine($"ROUTER >>>>>>>>>>>>>>>>>>>>>>>>>>>>>  Room id: {roomId}");
                        }
                ...
                catch
                {
                    ...
                    {
                        var roomId = parameters.Get<Guid>("RoomId");
                        System.Diagnostics.Debug.WriteLine($"ROUTER THROW >>>>>>>>>>>>>>>>>>>>>>>>>>>>>  Room id: {roomId}");
                    }
                    throw;
                }
                finally
                {
                    ...
                    {
                        var roomId = parameters.Get<Guid>("RoomId");
                        System.Diagnostics.Debug.WriteLine($"ROUTER RELEASING LOCK >>>>>>>>>>>>>>>>>>>>>>>>>>>>>  Room id: {roomId}");
                    }
                    _routerLock.Release();
                }
            }
